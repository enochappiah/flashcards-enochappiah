  ...
  return (
    <div className="border-b">
      <div className="relative w-3/4 h-48 mx-auto mt-5 bg-gray-200 border rounded shadow-lg sm:w-72 md:w-96 sm:h-56 md:h-64 mb-7">
        <div className={`absolute top-0 left-0 w-full h-full bg-white rounded p-4 ${isFlipped ? "hidden" : ""} `} >
        <CardActions cardId={card.id} cardFront={card.front} cardBack={card.back} isFlipped={isFlipped}/>
        ...
        <div className={`absolute top-0 left-0 w-full h-full bg-black rounded p-4  ${isFlipped ? "" : "hidden"}`}>
          <CardActions cardId={card.id} cardFront={card.front} cardBack={card.back} isFlipped={isFlipped}/>
